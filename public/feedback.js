...
async function feedback(){
    let show_code = localStorage.getItem("current_code2")
    try { 
        const response1 = await fetch(`/api/getData?code=${show_code}`); 
        if (!response1.ok) {
            throw new Error(`HTTP error! status: ${response1.status}`);
        }
        const data = await response1.json();
        let feedback = document.getElementById("message").value
        const response2 = await fetch(`/api/upDateFeedbacks?code=${show_code}`, { 
            method: 'POST',
            headers: {
